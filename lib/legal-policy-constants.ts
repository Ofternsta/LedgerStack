...
export const PLAN_TIER_LEGAL_SUMMARIES = [
  `Trial (7 days, card required) — ${PLAN_STAFF_LIMITS_DISPLAY.trial} (admin only; no worker or client accounts), ${PLAN_PROJECT_LIMITS_DISPLAY.trial}, ${PLAN_AI_LIMITS_DISPLAY.trial} AI summaries and Project AI chat replies per calendar month (shared pool), images/PDF uploads only; no exports, client portal, e-signatures, calendar, or team chat.`,
  `Starter ($20/month) — ${PLAN_STAFF_LIMITS_DISPLAY.starter} (admin only; no worker or client accounts), ${PLAN_PROJECT_LIMITS_DISPLAY.starter}, ${PLAN_AI_LIMITS_DISPLAY.starter} AI summaries and Project AI chat replies per calendar month (shared pool), standard PDF export, automatic backups (${BACKUP_LIMITS_DISPLAY.starter} retained); no client portal or e-signatures.`,
  `Professional ($70/month) — up to ${PLAN_ENTITLEMENTS.professional.maxStaffUsers} staff users, ${PLAN_PROJECT_LIMITS_DISPLAY.professional}, ${PLAN_AI_LIMITS_DISPLAY.professional} AI summaries and Project AI chat replies per calendar month (shared pool), client portal, client e-signatures (SignWell), calendar, internal notes, team messages, job packet exports, analytics, automatic backups (${BACKUP_LIMITS_DISPLAY.professional} retained).`,
  `Enterprise ($150/month) — ${PLAN_STAFF_LIMITS_DISPLAY.enterprise}, ${PLAN_PROJECT_LIMITS_DISPLAY.enterprise}, ${PLAN_AI_LIMITS_DISPLAY.enterprise} AI summaries and Project AI chat replies per calendar month, client portal, client e-signatures (SignWell), advanced analytics, automatic backups (${BACKUP_LIMITS_DISPLAY.enterprise} retained), priority support.`,
] as const
